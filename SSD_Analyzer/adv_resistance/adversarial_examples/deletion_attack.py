...
class DeletionToxicObjective(ToxicObjective):
  ...
  def __init__(self, model, input: str,  max_perturbs: int, del_chr: str = BKSP, ins_chr_min: str = '!', ins_chr_max: str = '~'):
...
for texts in test_data2:
    text = texts[0]
    # text = texts[1]
    obj = DeletionToxicObjective(model2, text, max_p)
...
with open('./langs/adv_deletion_test_{}.csv'.format(max_p), 'w') as f:
