...
langdata = pd.read_csv("data/model_2018.csv")
...
data["train"]["texts"] = rest_texts
data["dev"]["texts"] = test_texts
data["test"]["texts"] = test_texts

data["train"]["labels"] = rest_labels
...
test_features = convert_examples_to_inputs(data["test"]["texts"], 
                                           data["test"]["labels"], 
                                           label2idx, MAX_SEQ_LENGTH, tokenizer)
...
def evaluate_all_languages(model,device):
    _, correct_labels, predicted_labels = evaluate(model, data["test"]["dataloader"], device=device)
...
def train(model, train_dataloader, dev_dataloader, device,output_model_file="./bert.bin",
    ...
    num_train_steps = int(len(data["train"]["texts"]) / batch_size / gradient_accumulation_steps * num_train_epochs)
...
model_file_name = train(model, dataloader_train_all, dataloader_dev_all, device, gradient_accumulation_steps=4, num_train_epochs=20, 
                        output_model_file="./bert_model_2018.bin")

from transformers import BertForSequenceClassification

# Just right before the actual usage select your hardware
device = torch.device('cuda') # or cpu
    # send your model to your hardware
...
model2.load_state_dict(torch.load('./bert_uci.bin', map_location=torch.device('cuda')))
